...
- 仓库能用一套 npm 命令完成 lint / typecheck / test / Prisma schema validate。
- 本地 Postgres 可通过 Docker Compose 启动。
- Prisma schema 存在并能 `prisma validate`。
...
- [ ] Add package scripts:
  ...
  - `verify`: `npm run lint && npm run typecheck && npm run prisma:validate && npm run test`
- [ ] Add dependencies:
...
- [ ] Add Docker Compose service `postgres` with database name `digital_asset_dev`.
...
- [ ] Add package scripts:
  - `worker`: `tsx src/server/worker/index.ts`
  - `worker:smoke`: `tsx src/server/worker/index.ts --once --smoke`
